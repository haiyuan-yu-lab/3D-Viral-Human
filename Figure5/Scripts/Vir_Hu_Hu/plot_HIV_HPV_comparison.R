...
library(rstatix)
library(dplyr)
plot_df <- fread('/ix/djishnu/Priyamvada/viral-human-ppi/Github/3D-Viral-Human/Figure5/Data/Vir_Hu_Hu/HIV_HPV_comparison.txt.gz', header = T)
output_dir <- '/ix/djishnu/Priyamvada/viral-human-ppi/Github/3D-Viral-Human/Figure5/Plots/Vir_Hu_Hu'
stat_test <- plot_df %>%
...
names(colors) <- c('HIV','High risk')
bxp <- ggplot(
  plot_df_filtered,
  aes(x = Virus, y = Dice_index)) +
  geom_violin(trim = TRUE, aes(fill = Virus)) +
  geom_boxplot(width = 0.075, notch = TRUE, outliers = FALSE, fill = 'white') +
  scale_fill_manual(values = colors) +
  scale_x_discrete(breaks=c('High risk', 'HIV'),
                   labels=c("High risk HPV", "HIV")) +
